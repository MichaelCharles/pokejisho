...
function renderResult(array) {
  ...
  for (i = 0; i < array.length; i++) {
    ...
    if (result.type === "move") {
      result.url =
        "https://bulbapedia.bulbagarden.net/wiki/" + result.us + "_(move)";
      ...
      result.url =
        "https://bulbapedia.bulbagarden.net/wiki/" + result.us + "_(Ability)";
    } else {
      result.url = result.type;
    }
    $resultRow = $("<a href='" + result.url + "' target='_blank'></a>");
    $resultRow.html(
      '<div class="result"><div class="english-result">' +
        result[lang1] +
        '</div><div class="japanese-result">' +
        result[lang2] +
        "</div></div>"
    );
